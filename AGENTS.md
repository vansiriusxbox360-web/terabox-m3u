...
| **Vishnu III el Restaurador** | 25/08/2026 | Príncipe Vishnu II el Digno | Corrigió carátulas de series/pelis desconfiguradas. Causa raíz: claves de `STATION_POSTER_OVERRIDES` por subcadena (p.ej. 'Oscar') pisaban estaciones no relacionadas (Que Bello Es Sobrevivir, The Magic Pear Tree). Añadió claves específicas ANTES de las genéricas (el loop rompe en el primer match). Arregló: Asterix la batalla de los jefes (vuelve al custom-poster, no al del juego), Juego de Tronos y La casa del dragón (las carpetas ALB madre ahora consultan el póster forzado de las stations de sus hijos en `resolve_icon` all_alb en vez del fotograma del JSON), Spider-Man, Los ladrones van a la oficina, Que Bello Es Sobrevivir, The Magic Pear Tree, El Castillo en el cielo (Tenkū no Shiro Laputa → jbhifi), Tenshi no Tamago (ALB→amazon, hija→gstatic). v1.3.51. |
| **Vishnu IV el Emulador** | 01/09/2026 | Vishnu III el Restaurador | Probó añadir **ScummVM** al addon para juegos de aventuras (portable descargado desde Terabox como un juego más, routing por nombre/engine). **Descartado y revertido**: los juegos de aventuras pesan cientos de MB y cargarlos desde Kodi no es viable para VanSirius. Se revirtió `index.js`/`default.py` a v1.3.54 y se borró el portable de Terabox. Si algún día se retoma: el portable se sube con un script tipo `subir_lote4.js` (no a GitHub) y se expone su path en el JSON. |
